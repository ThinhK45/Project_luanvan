    ...
    const updateDispatch = async (name, data) => {
        switch (name) {
            case 'account': {
                ...
                //get count orders
                try {
                    const res1 = await countOrder('Đã nhận hàng', _id, '');
                    const res2 = await countOrder('Hủy đơn', _id, '');
            ...
            case 'vendor': {
                ...
                //get count orders
                try {
                    const res1 = await countOrder('Đã nhận hàng', '', data._id);
                    const res2 = await countOrder('Hủy đơn', '', data._id);
                    data.numberOfSucessfulOrders = res1.count;
                    data.numberOfFailedOrders = res2.count;
                } catch {
                    data.numberOfSucessfulOrders =
